    ...
    <x-tabs-event.layout>
        <div class="flex justify-between">
            ...
            <div class='flex flex-col gap-2 lg:flex-row'>
                <flux:input size='xs' icon="magnifying-glass" wire:model.live='search_event_type' placeholder="Search Event Type" />
                <flux:select size="xs" wire:model.live="search_event_category" placeholder="Choose Event Category...">
                    @foreach ($EventCategory as $event_categories)
                    <flux:select.option value="{{ $event_categories->id }}">{{ $event_categories->event_category_name }}</flux:select.option>
                    @endforeach
                </flux:select>
            </div>
        </div>
        <div class="overflow-x-auto ">
            <table class="table table-xs">
                ...
                <tbody class="text-center">
                    @foreach ($EventType as $no => $event_type)
                    <tr>
                        <th>{{ $EventType->firstItem() + $no }}</th>
                        <th>{{ $event_type->event_type_name }}</th>
                        <th>{{ $event_type->EventCategories->event_category_name }}</th>
        ...
        <div class="mt-4">{{ $EventType->links() }}</div>
        <flux:modal name="EventType">
            <form wire:submit.prevent='store' class='grid justify-items-stretch'>
                ...
                <fieldset class="fieldset bg-base-200 border-base-300 rounded-box w-xs sm:w-sm border p-4 sm:justify-self-center">
                    ...
                    <x-text-input wire:model.live='event_type_name' :error="$errors->get('event_type_name')" type="text" placeholder="Nama Event Category" />
                    <x-label-error :messages="$errors->get('event_type_name')" />
                    {{-- Event Category --}}
                    <x-label-req>{{ __('event category') }} </x-label-req>
                    <flux:select size="xs" wire:model.live="event_category_id" placeholder="Choose Event Category...">
                        @foreach ($EventCategory as $event_categories)
                        <flux:select.option value="{{ $event_categories->id }}">{{ $event_categories->event_category_name }}</flux:select.option>
                        @endforeach
                    </flux:select>
                    <x-label-error :messages="$errors->get('event_category_id')" />
                    {{-- Status --}}
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <flux:text class="mt-2">
                        <p>You're about to delete this {{ $event_type_name }}.</p>
