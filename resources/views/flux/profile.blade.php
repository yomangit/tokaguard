...
<button type="button" {{ $attributes->class($classes) }} data-flux-profile>
    <div class="shrink-0">
        <?php if ($avatar instanceof \Illuminate\View\ComponentSlot): ?>
            {{ $avatar }}
        <?php else: ?>
            <flux:avatar :attributes="Flux::attributesAfter('avatar:', $attributes, ['src' => $avatar, 'size' => 'xs', 'circle' => $circle, 'name' => $name, 'initials' => $initials])" />
        <?php endif; ?>
    </div>

    <?php if ($name): ?>
        <span class="mx-1 text-xs text-zinc-500 dark:text-neutral-content group-hover:text-zinc-800 dark:group-hover:text-neutral font-medium capitalize truncate">
            {{ $name }}
        </span>
    <?php endif; ?>
